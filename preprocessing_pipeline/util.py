...
def random_data_split(path=".\\"): #random test-train-validate datasets
    ...
    noise_noise = glob(f"{path}\\spectogram_data\\Random_Noise\\noise\\*")
    ...
    #noise_people
    train['length'] =  int(len(noise_people)*0.7)
    test['length'] = int(len(noise_people)*0.3)
    validate['length'] = len(noise_people) - train['length'] - test['length']
